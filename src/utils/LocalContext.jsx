...
const Contexto = createContext({
  localId: null, local: null, cargando: true,
...
export function LocalProvider({ children }) {
  ...
  useEffect(() => {
    if (!localId) { setLocal(null); setCargando(false); return }
    ...
    if (!user) { setLocal(null); setCargando(false); return }

    setCargando(true)
    const unsub = suscribirLocal(localId, (datos) => {
      setLocal(datos)
      setCargando(false)
    })
    return unsub
